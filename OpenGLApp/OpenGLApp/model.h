...
struct TextureInfo { unsigned int id; int w, h; };

TextureInfo TextureFromFile(const char* path,
...
private:
    ...
    Mesh processMesh(aiMesh* mesh, const aiScene* scene)
    {
        ...
        // get diffuse color from material (.mtl)
        glm::vec3 diffuseColor(1.0f, 1.0f, 1.0f); // default white
        aiColor3D color(0.f, 0.f, 0.f);
        if (material->Get(AI_MATKEY_COLOR_DIFFUSE, color) == AI_SUCCESS) {
            diffuseColor = glm::vec3(color.r, color.g, color.b);
        }

        // return mesh with diffuse color
